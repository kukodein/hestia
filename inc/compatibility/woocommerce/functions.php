...
function hestia_woocommerce_before_main_content() {
	...
	echo '<div class="blog-post arsip-produk">';
...
function hestia_woocommerce_template_loop_product_title() {
	...
	<div class="content">
		...
		if ( $post->post_excerpt ) :
			...
			?>
			<div class="card-description" style="display:none!important;"><?php echo wp_kses_post( apply_filters( 'woocommerce_short_description', $limited_excerpt ) ); ?></div>
		...
		<div class="footer">
			...
			if ( ! empty( $product_price ) ) {
				...
				echo '<div class="price" style="text-align:center;"><h4>';
			...
			?>
			<div class="stats" style="display:none!important;">
				<?php hestia_add_to_cart(); ?>
			</div>
			
		</div>
		<div class="archive-ecommerce-button" style="margin-top:10px;">
		
		<?php 

		$img_tokopedia = get_field('tokopedia_image','option');
		$img_shopee    = get_field('shopee_image','option');

		if ($img_tokopedia == false) { $img_tokopedia = "https://placehold.co/50?text=Tokoped"; }
		if ($img_shopee == false) { $img_shopee = "https://placehold.co/50?text=Shopee"; }

		$url_tokopedia = get_field('tokopedia_product_url');
		$url_shopee    = get_field('shopee_product_url');

		if ($url_tokopedia != false) {
			echo '<a href="' . $url_tokopedia . '" target="_blank"><img src="' . $img_tokopedia . '"></a>';
		}

		print "\n";

		if ($url_shopee != false) {
			echo '<a href="' . $url_shopee . '" target="_blank"><img src="' . $img_shopee . '"></a>';
		}
